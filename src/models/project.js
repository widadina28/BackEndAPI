...
return new Promise((resolve, reject) => {
  db.query(`SELECT project.project_name, project.description, project.deadline, project.image, company.name_company, project.createAt, project.updateAt, project.price, project.id_project, (SELECT COUNT(*) FROM project) as count FROM project JOIN company ON project.id_company = company.id_company WHERE project.id_company =${id}`, (err, result, _field) =>{
    if (err) {
      reject(new Error(err))
    } else {
      resolve(result)
